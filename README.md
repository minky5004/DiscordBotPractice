...
기능이 들어온 순서 — `/ping` 슬래시 명령 → 엔케팔린 완충 멘션 예약(PostgreSQL 영속화 · Docker) → Steam 공지 중계 →
정기 점검 알림(공지 이미지 OCR) → 인격 · E.G.O · 기프트 · 환상체 · 키워드 조회(Components V2 화면).
...
| --- | --- | --- |
| `/ping` | 연결 확인 | `Pong!` |
| `/help` | 부른 사람이 쓸 수 있는 명령 전부 · 필수 옵션 자리 | 개인 메시지 한 통 · DM 을 막아 둔 사람에게는 나만 보기 응답 |
...
| `/환상체 이름:징벌 새` | 환상체 80종 조회 · 한국어 이름 · 영어 문서 제목 자동완성 | 위험 등급 색 띠 · 유래 E.G.O · 얻는 기프트 · 등장 거울 · 관찰 로그 버튼 |
...
기프트의 죄악 · 등급 · 가격 · 키워드는 위키 데이터 모듈(`Module:EgoGift/data`) 한 페이지 — 기프트 401종 전부 영어 이름으로 짝.
강화판에서 바뀐 수치는 게임 텍스트의 강조 표시 그대로 굵은 글씨.
...
환상체만 목록의 출처가 위키 쪽 — `{{AbnoInfo}}` 를 쓰는 문서 80종 · 번역 하위문서 제외 · 게임 도감에서 가져오는 것은 한국어 관찰 로그뿐.
도감 항목이 있는 쪽은 39종 — 나머지는 거울 던전 이벤트에서만 만나는 · 도감에 오르지 않는 환상체.
